...
Corretto; il fallback AVX2 e' **bit-identico** al kernel a token singolo. I nuovi
percorsi VNNI mantengono la stessa aritmetica intera e differiscono solo per
l'ordine della riduzione fp32 (entro `1e-4`, verificato sotto).
...
**4. IQ2_XXS riscritto e tenuto: AVX-512 VNNI batch.**  IQ2_XXS e' il tipo di
**9/48 layer** ISTA.  Anche qui un zmm sostituisce le due catene ymm: le otto
righe di griglia e gli otto pattern di segno sono assemblati una volta, poi
`vpmaddubs + vpdpwssd` usa un accumulatore int32 per token.  A/B nello stesso
processo: **25,47 -> 47,91 GMAC/s L1-hot (+88%)**, **20,95 -> 30,24 streamed
(+44%)**.  Nel modello (IQ3_S VNNI gia' attivo), sequenza AVX2 -> VNNI -> VNNI
-> AVX2: mid medio **496,3 -> 452,1 ms/layer (-8,9%)**, prefill medio
**49,13 -> 53,09 t/s (+8,1%)**.  Fallback: `DS4_QWEN4_IQ2XXS_VNNI=0`.

**5. IQ2_XS riscritto e tenuto: AVX-512 VNNI batch.** IQ2_XS e' il tipo di
**10/48 layer** ISTA. Per ogni passo di 128 valori, due zmm coprono 64 valori
ciascuno: 16 griglie e 16 pattern di segno sono costruiti una volta, mentre le
scale a nibble (16 valori ciascuna) entrano nei due `vpdpwssd`. Due A/B
nello stesso processo: **25,38 -> 36,09 GMAC/s L1-hot (+42%)**, **18,89 ->
25,34 streamed (+34%)**. Nel modello con IQ3_S/IQ2_XXS VNNI gia' attivi, 8 run
alternati danno mid medio **426,4 -> 411,4 ms/layer (-3,5%)** e prefill medio
**55,92 -> 56,90 t/s (+1,7%)**; il singolo run resta molto sensibile al warm-up.
Fallback: `DS4_QWEN4_IQ2XS_VNNI=0`.

**6. IQ3_XXS riscritto e tenuto: AVX-512 VNNI batch.** IQ3_XXS e' il tipo di
**6/48 layer** ISTA. Il suo passo da 64 valori contiene 16 griglie da 4 byte,
8 pattern di segno e due scale da 32: entra esattamente in uno zmm, poi
`vpmaddubs + vpdpwssd` sostituisce due catene AVX2. Due A/B nello stesso
processo danno media **23,06 -> 32,95 GMAC/s L1-hot (+43%)**, **19,12 ->
24,56 streamed (+29%)**. Nel modello, dopo il warm-up, VNNI `c1` contro AVX2
...
- offset aggiunto; i cinque fallback AVX2 danno `0.00e+00` contro il kernel a
  token singolo (bit-identici). I default VNNI successivi di IQ3_S/IQ2_XXS sono
  entro `1e-4`, con la sola diversa associazione della riduzione fp32;
...
**Stato della copertura dei kernel di produzione** (tutti confrontati col percorso
a token singolo, che e' validato contro un riferimento indipendente in doppia
precisione; AVX2/fp32 esatti, VNNI entro `1e-4`):

| kernel batch | tipo | layer ISTA | esito |
|---|---|---|---|
| `iq3_s_q8k_vnni_batch` | IQ3_S | 13 | ok (16 casi `<1e-4`; AVX2 fallback esatto) |
| `iq2_xs_q8k_vnni_batch` | IQ2_XS | 10 | ok (16 casi `<1e-4`; AVX2 fallback esatto) |
| `iq2_xxs_q8k_vnni_batch` | IQ2_XXS | 9 | ok (16 casi `<1e-4`; AVX2 era **rotto**, corretto) |
...
### Mappa pre-riscritture VNNI (storica; vedere l'indice per i valori correnti)
...
`time_q8k_batch_ab()` alterna AVX2/VNNI in ordine inverso per sei passaggi;
...
- `check_q8k_batch_variants()` confronta **sia** AVX2 sia VNNI contro il kernel
  q8k a token singolo su 16 pesi/attivazioni variati; AVX2 `0.00e+00`, VNNI
...
## IQ2_XXS: riscrittura VNNI batch, vincente e integrata

IQ2_XXS ha otto righe di griglia da 8 byte per passo di 64 valori e otto indici
separati di segno a 7 bit. Il nuovo batch le unisce in un solo zmm (lane basse:
primi 32 valori; lane alte: secondi 32), trasforma gli otto pattern `+/-1` in
una mask byte e fa `vpmaddubs -> vpdpwssd` con le due scale. Come IQ3_S, questo
porta da due accumulatori ymm a un accumulatore zmm per token.

### A/B del kernel (stesso processo)

```
IQ2_XXS A/B maddubs x8 nbuf=8    L1-hot : 25.47 GMAC/s
IQ2_XXS A/B vnni    x8 nbuf=8    L1-hot : 47.91 GMAC/s  (+88%)
IQ2_XXS A/B maddubs x8 nbuf=4096 streamed: 20.95 GMAC/s
IQ2_XXS A/B vnni    x8 nbuf=4096 streamed: 30.24 GMAC/s (+44%)
```

### A/B nel modello ISTA (IQ3_S VNNI gia' attivo, T=1739)

| run | IQ2_XXS VNNI | mid ms/layer | down ms/layer | totale ms/layer | prefill |
|---|---:|---:|---:|---:|---:|
| a0 | 0 | 515,5 | 203,0 | 734,8 | 47,55 t/s |
| b1 | 1 | 444,1 | 191,3 | 651,3 | 53,46 t/s |
| c1 | 1 | 460,0 | 184,6 | 660,9 | 52,71 t/s |
| d0 | 0 | 477,1 | 193,3 | 686,5 | 50,71 t/s |

La macchina migliora parzialmente dopo il primo run, quindi non si confrontano
solo gli estremi: VNNI batte l'AVX2 adiacente in entrambi i lati della sequenza
(a0->b1 e c1->d0). Media indicativa: **mid -8,9%, prefill +8,1%**.

### Correttezza

- `check_q8k_batch_variants()` verifica sia `maddubs` sia VNNI per IQ3_S,
  IQ2_XXS, IQ2_XS e IQ3_XXS su 16 casi; VNNI resta `<1e-4` e AVX2 e'
  `0.00e+00`.
- Dispatch default verificato; `DS4_QWEN4_IQ2XXS_VNNI=0` e
  `DS4_QWEN4_IQ2XS_VNNI=0` ripristinano AVX2 e passano.
- `make test-qwen4-cpu-dot`, build `ds4`/`ds4-server`, e `391` su ISTA+UD: verdi.

## IQ2_XS: riscrittura VNNI batch, vincente e integrata

IQ2_XS usa 32 entry da 16 bit per blocco: 9 bit selezionano una griglia di 8
byte e 7 bit un pattern di segno. In un passo da 128 valori il percorso nuovo
compone due zmm (64 valori ciascuno), crea una mask byte di segno per zmm e
applica `vpmaddubs -> vpdpwssd`. Le scale non sono uniformi su 32 valori: ogni
byte ha nibble basso/alto per due meta' da 16; il nuovo codice combina due
`g_iq2s_sc16[]` in ogni zmm per conservare esattamente quella disposizione.

### A/B del kernel (due run, stesso processo per ciascun A/B)

```
run 1 hot:      26.41 -> 36.92 GMAC/s  (+40%)
run 1 streamed: 18.92 -> 25.23 GMAC/s  (+33%)
run 2 hot:      24.34 -> 35.25 GMAC/s  (+45%)
run 2 streamed: 18.86 -> 25.44 GMAC/s  (+35%)
media: hot 25.38 -> 36.09 (+42%), streamed 18.89 -> 25.34 (+34%)
```

### Campagna modello ISTA (T=1739; IQ3_S/IQ2_XXS VNNI gia' attivi)

| run | IQ2_XS VNNI | mid ms/layer | down ms/layer | totale ms/layer | prefill |
|---|---:|---:|---:|---:|---:|
| a0 | 0 | 469,1 | 198,0 | 683,1 | 51,00 t/s |
| b1 | 1 | 435,5 | 189,0 | 640,2 | 54,31 t/s |
| c1 | 1 | 421,3 | 187,1 | 624,5 | 55,10 t/s |
| d0 | 0 | 424,7 | 180,4 | 621,1 | 55,99 t/s |
| e1 | 1 | 405,9 | 178,7 | 600,9 | 57,56 t/s |
| f0 | 0 | 403,6 | 172,3 | 591,8 | 58,67 t/s |
| g0 | 0 | 408,3 | 174,3 | 598,5 | 58,03 t/s |
| h1 | 1 | 383,0 | 173,0 | 571,8 | 60,62 t/s |

C'e' warm-up reale del processo/cache: i tempi assoluti calano lungo la serie,
quindi non si attribuisce la punta finale al solo kernel. Sui quattro run per
variante il mid medio e' **426,4 -> 411,4 ms/layer (-3,5%)**; tre delle quattro
transizioni adiacenti favoriscono VNNI (la quarta e' -0,6%). Il guadagno
microkernel e' molto piu' stabile; il prefill medio e' **55,92 -> 56,90 t/s
(+1,7%)**, da leggere come indicativo, non come promessa assoluta.

### Correttezza

- `check_q8k_batch_variants()` ora varia 16 volte **sia** pesi sia attivazioni,
  e confronta VNNI/maddubs con il singolo per tutti e quattro i nuovi tipi;
  VNNI `<1e-4`, AVX2 `0.00e+00`.
- Dispatch default e fallback `DS4_QWEN4_IQ2XS_VNNI=0` sono entrambi verdi.
- `make test-qwen4-cpu-dot`, build `ds4`/`ds4-server`, e `391` su ISTA+UD: verdi.

## IQ3_XXS: riscrittura VNNI batch, vincente e integrata

Per ogni passo IQ3_XXS da 64 valori ci sono 16 griglie da 4 byte, otto pattern
di segno da 8 byte e due scale dispari da 32 valori. Il percorso VNNI li fonde
in uno zmm: i 16 `uint32` della griglia formano le lane, gli otto pattern
formano una mask byte e `vpmaddubs -> vpdpwssd` riduce direttamente in 16 lane
int32 per token. Il fattore finale resta `0.25`, come il gemello AVX2.

### A/B del kernel (due run, stesso processo per ciascun A/B)

```
run 1 hot:      23.78 -> 34.94 GMAC/s  (+47%)
run 1 streamed: 19.39 -> 25.85 GMAC/s  (+33%)
run 2 hot:      22.33 -> 30.95 GMAC/s  (+39%)
run 2 streamed: 18.84 -> 23.26 GMAC/s  (+23%)
media: hot 23.06 -> 32.95 (+43%), streamed 19.12 -> 24.56 (+29%)
```

### Campagna modello ISTA (T=1739; gli altri quattro tipi q8k gia' VNNI)

| run | IQ3_XXS VNNI | mid ms/layer | down ms/layer | totale ms/layer | prefill |
|---|---:|---:|---:|---:|---:|
| a0 | 0 | 488,0 | 211,0 | 715,8 | 48,61 t/s |
| b1 | 1 | 398,1 | 183,7 | 598,3 | 57,96 t/s |
| c1 | 1 | 391,4 | 183,2 | 591,1 | 58,62 t/s |
| d0 | 0 | 435,8 | 184,5 | 636,7 | 54,65 t/s |

Il primo AVX2 e' freddo, quindi il confronto piu' utile e' `c1 -> d0`: anche
dopo il warm-up spegnere solo IQ3_XXS VNNI peggiora il **mid del 10,2%** e il
prefill del **7,3%**. Il segno coincide con entrambi gli A/B interni.

### Correttezza

- `check_q8k_batch_variants()` su 16 pesi/attivazioni: IQ3_XXS VNNI `<1e-4`,
