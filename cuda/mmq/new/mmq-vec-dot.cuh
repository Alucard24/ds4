...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q4_0_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q4_0, I);
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q4_1_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q4_1, I);
    const int   * x_qs = (const int   *) x;
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q8_0_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q8_0, I);
    const int   * x_qs = (const int   *) x;
    const float * x_df = (const float *) x_qs + txs.qs;
    const int   * y_qs = (const int   *) y + 4;
...
template <ggml_type type, int J, bool fallback, ds4n_mmq_q8_1_ds_layout ds_layout>
static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q8_0_q8_1_mma(
    ...
    y += (threadIdx.y % ntx) * (ds4n_tile_C::J*MMQ_TILE_Y_K);
...
#pragma unroll
        for (int j0 = 0; j0 < J; j0 += ntx*ds4n_tile_C::J) {
            ...
            float dB;
            const int j = j0 + ds4n_tile_C::get_j(0);
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    const int i = i0 + n*tile_A::I + ds4n_tile_C::get_i(l);
                    const float dA = x_df[i*sram_stride + k0/QI8_0];
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += C.x[l]*dA*dB;
                }
            }
        }
    }
    ...
    float dA[ntx][ds4n_tile_C::ne/2][MMQ_TILE_NE_K/QI8_0];
...
#pragma unroll
        for (int l = 0; l < ds4n_tile_C::ne/2; ++l) {
            const int i = i0 + n*tile_A::I + ds4n_tile_C::get_i(2*l);
...
#pragma unroll
        for (int k01 = 0; k01 < MMQ_TILE_NE_K; k01 += QI8_0) {
            tile_B B;
            float dB[ds4n_tile_C::ne/2];

            load_generic(B, y_qs + j0*MMQ_TILE_Y_K + k01, MMQ_TILE_Y_K); // faster than load_ldmatrix

#pragma unroll
            for (int l = 0; l < ds4n_tile_C::ne/2; ++l) {
                const int j = j0 + ds4n_tile_C::get_j(l);
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += C.x[l]*dA[n][l/2][k01/QI8_0]*dB[l%2];
                }
            }
        }
    }
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q8_1_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q5_1, I);
    const int   * x_qs = (const int   *) x;
    const half2 * x_dm = (const half2 *) x_qs + txs.qs;
    const int   * y_qs = (const int   *) y + 4;
    const half2 * y_ds = (const half2 *) y;
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    ...
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += dmA.x*dsB.x*C.x[l];
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += dmA.y*dsB.y;
                }
            }
        }
    }
    ...
    float2 dmA[ntx][ds4n_tile_C::ne/2][MMQ_TILE_NE_K/QI8_1];
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += dmA[n][l/2][k01/QI8_1].x*dsB[l%2].x*C.x[l];
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += dmA[n][l/2][k01/QI8_1].y*dsB[l%2].y;
                }
            }
        }
    }
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q8_0_16_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(type, I);
    const int   * x_qs = (const int   *) x;
    const float * x_df = (const float *) x_qs + txs.qs;
    const int   * y_qs = (const int   *) y + 4;
    const float * y_df = (const float *) y;
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    ...
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += C.x[l] * x_df[i*sram_stride + k0/4] * dB;
                }
            }
        }
    }
    ...
    tile_A  A[ntx][8];
    float  dA[ntx][ds4n_tile_C::ne/2][8];
...
#pragma unroll
        for (int l = 0; l < ds4n_tile_C::ne/2; ++l) {
            const int i = i0 + n*ds4n_tile_C::I + ds4n_tile_C::get_i(2*l);
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += dB[l%2]*(C[0].x[l]*dA[n][l/2][k01/4 + 0] + C[1].x[l]*dA[n][l/2][k01/4 + 1]);
                }
            }
        }
    }
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q2_K_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q2_K, I);
    const int   * x_qs = (const int   *) x;
    const half2 * x_dm = (const half2 *) x_qs + txs.qs;
    const int   * y_qs = (const int   *) y + 4;
    const half2 * y_ds = (const half2 *) y;
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    ...
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += tmp*dB;
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] -= dm.y*sB;
                }
            }
        }
    }
    ...
    tile_A  A[ntx][8];
    float  dA[ntx][ds4n_tile_C::ne/2][8];
    float  mA[ntx][ds4n_tile_C::ne/2][8];
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    ...
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += tmp*(k01 < MMQ_TILE_NE_K/2 ? dB[l%2].x : dB[l%2].y);
                }
            }
        }
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] -= mA[n][l/2][k01/4 + 0]*sB[l%2].x;
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] -= mA[n][l/2][k01/4 + 1]*sB[l%2].y;
                }
            }
        }
    }
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q3_K_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q3_K, I);
    const int   * x_qs = (const int   *) x;
    const float * x_df = (const float *) x_qs + txs.qs;
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q4_K_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q4_K, I);
    const int   * x_qs = (const int   *) x;
    const half2 * x_dm = (const half2 *) x_qs + txs.qs;
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q5_K_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q5_K, I);
    const int   * x_qs = (const int   *) x;
    const half2 * x_dm = (const half2 *) x_qs + txs.qs;
    const int   * x_sc = (const int   *) x_dm + txs.dm;
    const int   * y_qs = (const int   *) y + 4;
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_q6_K_q8_1_dp4a(
    ...
    constexpr ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(GGML_TYPE_Q6_K, I);
    const int   * x_qs = (const int   *) x;
    const float * x_df = (const float *) x_qs + txs.qs;
    const int   * x_sc = (const int   *) x_df + txs.dm;
    const int   * y_qs = (const int   *) y + 4;
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    ...
                    sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += C.x[l] * sc[k01/4] * x_df[i*sram_stride] * dB;
                }
            }
        }
    }
    ...
    tile_A   A[ntx][8];
    int    scA[ntx][ds4n_tile_C::ne/2][8];
    float   dA[ntx][ds4n_tile_C::ne/2];
...
#pragma unroll
    for (int j0 = 0; j0 < J; j0 += ntx*ds4n_tile_C::J) {
        float tmp[ntx][ds4n_tile_C::ne] = {{0.0f}};
...
#pragma unroll
            for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l] += tmp[n][l]*dA[n][l/2];
            }
        }
    }
#else
...
template <ggml_type type, int J, bool fallback> static __device__ __forceinline__ void ggml_cuda_mmq_vec_dot_fp4_fp4_mma(
    ...
    typedef tile<16, 8, int>   tile_A;
    typedef tile<8,  8, int>   tile_B;
    typedef tile<16, 8, float> ds4n_tile_C;

    constexpr int sram_stride   = ggml_cuda_mmq_get_sram_stride(type, J, fallback);
    constexpr int rows_per_warp = ggml_cuda_mmq_get_rows_per_warp(type, J, fallback);
    constexpr int ntx           = rows_per_warp / ds4n_tile_C::I;
...
#pragma unroll
                for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                    sum[(j0 / ds4n_tile_C::J + n) * ds4n_tile_C::ne + l] += C.x[l];
                }
            }
        }
    }
