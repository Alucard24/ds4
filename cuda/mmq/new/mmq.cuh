...
struct ds4n_block_fp4_mmq {
    uint32_t d4[4];
    int8_t   qs[QK_FP4_MMQ / 2];
};

static_assert(sizeof(ds4n_block_q8_1_mmq) == QK8_1_MMQ + 4*sizeof(half2), "Unexpected ds4n_block_q8_1_mmq size");
static_assert(sizeof(ds4n_block_q8_1_mmq) == 4*sizeof(block_q8_1),      "Unexpected ds4n_block_q8_1_mmq size");
static_assert(sizeof(ds4n_block_fp4_mmq)  == sizeof(ds4n_block_q8_1_mmq),    "Unexpected ds4n_block_fp4_mmq size");

static ds4n_mmq_q8_1_ds_layout ds4n_mmq_get_q8_1_ds_layout(const ggml_type type_x) {
    switch (type_x) {
        case GGML_TYPE_Q1_0:
        case GGML_TYPE_Q2_0:
...
// ds4n_block_q8_1_mmq has (128 8-bit ints == 32 32-bit ints + 4 32-bit scales)
...
static __host__ ggml_cuda_mmq_config ds4n_mmq_get_config(const ggml_type type, const int J, const bool fallback, const int cc) {
...
static constexpr __device__ ggml_cuda_mmq_config ds4n_mmq_get_config(ggml_type type, int J, bool fallback) {
#ifdef GGML_USE_HIP
#ifdef CDNA
    return ggml_cuda_mmq_get_config_cdna(type, J, fallback);
#elif defined(RDNA4)
...
static __host__ int ds4n_mmq_get_J_max(const ggml_type type, const bool fallback, const int cc, const int64_t ne11) {
    int ret = std::min(ne11, int64_t(512));
    ret -= ret % 8;
    for (;ret > 0; ret -= 8) {
        if (ds4n_mmq_get_config(type, ret, fallback, cc).type != GGML_TYPE_COUNT) {
            return ret;
        }
    }
    return ret;
...
static constexpr __device__ int ggml_cuda_mmq_get_rows_per_warp(ggml_type type, int J, bool fallback) {
    return ds4n_mmq_get_config(type, J, fallback).rows_per_warp();
}

#define MMQ_DP4A_TXS_Q4_0    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K   + I, I*MMQ_TILE_NE_K/QI4_0   + I/QI4_0,     0}
#define MMQ_DP4A_TXS_Q4_1    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K   + I, I*MMQ_TILE_NE_K/QI4_1   + I/QI4_1,     0}
#define MMQ_DP4A_TXS_Q8_0    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K*2/QI8_0 + I/(QI8_0/2), 0}
#define MMQ_DP4A_TXS_Q8_0_16 ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K*4/QI8_0 + I/(QI8_0/4), 0}
#define MMQ_DP4A_TXS_Q8_1    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K*2/QI8_1 + I/(QI8_1/2), 0}
#define MMQ_DP4A_TXS_Q2_K    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K         + I,           0}
#define MMQ_DP4A_TXS_Q3_K    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I,                                     I*MMQ_TILE_NE_K/8 + I/8}
#define MMQ_DP4A_TXS_Q4_K    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K   + I, I*MMQ_TILE_NE_K/QI4_K,                 I*MMQ_TILE_NE_K/8 + I/8}
#define MMQ_DP4A_TXS_Q5_K    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K/QI5_K   + I/QI5_K,     I*MMQ_TILE_NE_K/8 + I/8}
#define MMQ_DP4A_TXS_Q6_K    ds4n_tile_x_sizes{I*MMQ_TILE_NE_K*2 + I, I*MMQ_TILE_NE_K/QI6_K   + I/QI6_K,     I*MMQ_TILE_NE_K/8 + I/8}

static constexpr __host__ __device__ ds4n_tile_x_sizes ds4n_mmq_get_dp4a_tile_x_sizes(ggml_type type, int I) {
    switch (type) {
        ...
        default:                return ds4n_tile_x_sizes{0, 0, 0};
    }
}

// FIXME temporary until all combinations of data types and GPUs can use the MMA data layout
static __host__ int ds4n_mmq_get_nbytes_shared_x(const ggml_cuda_mmq_config & config, const int cc) {
    if (config.use_mma_data_layout(cc)) {
        return config.I * ggml_cuda_mmq_get_sram_stride(config.sram_layout) * 4;
    }
    const ds4n_tile_x_sizes txs = ds4n_mmq_get_dp4a_tile_x_sizes(config.type, config.I);
...
#if defined(AMD_MFMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)
    typedef tile<16, 16, int, DATA_LAYOUT_J_MAJOR> ds4n_tile_C;
#else
    typedef tile<16,  8, int> ds4n_tile_C;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(AMD_WMMA_AVAILABLE)

    constexpr int rows_per_warp = ggml_cuda_mmq_get_rows_per_warp(type, J, fallback);
    constexpr int ntx           = rows_per_warp/ds4n_tile_C::I; // Number of x minitiles per warp.

    const int i0 = (threadIdx.y / ntx) * (ntx*ds4n_tile_C::I);

    const bool y_scale_used = y_scale != nullptr;

#pragma unroll
    for (int j0 = 0; j0 < J; j0 += ntx*ds4n_tile_C::J) {
#pragma unroll
        for (int n = 0; n < ntx; ++n) {
#pragma unroll
            for (int l = 0; l < ds4n_tile_C::ne; ++l) {
                const int j = j0 + (threadIdx.y % ntx) * ds4n_tile_C::J + ds4n_tile_C::get_j(l);

                if (j > j_max) {
                    continue;
                }

                const int i = i0 + n*ds4n_tile_C::I + ds4n_tile_C::get_i(l);
                ...
                if constexpr (type == GGML_TYPE_NVFP4) {
                    if (y_scale_used) {
                        dst[ids_dst[j]*stride + i] = y_scale[j] * sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l];
                    } else {
                        dst[ids_dst[j]*stride + i] = sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l];
                    }
                } else {
                    dst[ids_dst[j]*stride + i] = sum[(j0/ds4n_tile_C::J + n)*ds4n_tile_C::ne + l];
                    GGML_UNUSED(y_scale_used);
...
static constexpr __device__ ggml_cuda_mmq_util_funcs ggml_cuda_mmq_get_util_funcs() {
    if (!ds4n_mmq_get_config(type, J, fallback).use_mma_data_layout()) {
        switch (type) {
            case GGML_TYPE_Q1_0:
...
static __device__ __forceinline__ void ds4n_mul_mat_q_process_tile(
    ...
    constexpr int sz = sizeof(ds4n_block_q8_1_mmq) / sizeof(int);
...
// The ds4n_mul_mat_q kernel implements "stream-k" work partitioning as described in https://arxiv.org/abs/2301.03598

template <ggml_type type, int J, bool fallback>
__launch_bounds__(ggml_cuda_mmq_get_nthreads(type, J, fallback), ggml_cuda_mmq_get_occupancy(type, J, fallback))
static __global__ void ds4n_mul_mat_q(
    ...
    if (ds4n_mmq_get_config(type, J, fallback).type == GGML_TYPE_COUNT) {
        NO_DEVICE_CODE;
        return;
    }
...
#pragma unroll
        ...
        offset_y   += (col_low + jt*J)*(sizeof(ds4n_block_q8_1_mmq)/sizeof(int));
        offset_dst += it*I;
        ...
        ds4n_mul_mat_q_process_tile<type, J, fallback, fixup>
...
static __global__ void ds4n_mul_mat_q_stream_k_fixup(
...
static size_t ds4n_mmq_get_nbytes_shared(const ggml_cuda_mmq_config & config, const int cc) {
    const size_t nbs_ids = config.J*sizeof(int);
    const size_t nbs_x = ds4n_mmq_get_nbytes_shared_x(config, cc);
    const size_t nbs_y = config.J * (sizeof(ds4n_block_q8_1_mmq));
    return nbs_ids + nbs_x + GGML_PAD(nbs_y, config.nthreads*sizeof(int));
}

template <ggml_type type, int J, bool fallback>
static void ds4n_launch_mul_mat_q(ggml_backend_cuda_context & ctx, const ds4n_mmq_args & args, cudaStream_t stream) {
    const int id = ggml_cuda_get_device();
    const int cc = ggml_cuda_info().devices[id].cc;
    const int nsm = ggml_cuda_info().devices[id].nsm;
    const int warp_size = ggml_cuda_info().devices[id].warp_size;

    const ggml_cuda_mmq_config config = ds4n_mmq_get_config(type, J, fallback, cc);
    GGML_ASSERT(config.nthreads % warp_size == 0);
    const int nwarps = config.nthreads / warp_size;
    const int nbytes_shared = ds4n_mmq_get_nbytes_shared(config, cc);

    const dim3 block_dims(warp_size, nwarps, 1);

    CUDA_SET_SHARED_MEMORY_LIMIT((ds4n_mul_mat_q<type, J, false>), nbytes_shared);
    CUDA_SET_SHARED_MEMORY_LIMIT((ds4n_mul_mat_q<type, J,  true>), nbytes_shared);
    ...
    if (!ggml_cuda_mmq_get_stream_k(type, J, fallback, cc)) {
        ds4n_mul_mat_q<type, J, fallback><<<block_nums_xy_tiling, block_dims, nbytes_shared, stream>>>
    ...
    ds4n_mul_mat_q<type, J, fallback><<<block_nums_stream_k, block_dims, nbytes_shared, stream>>>
    ...
    ds4n_mul_mat_q_stream_k_fixup<type, J, fallback><<<block_nums_fixup, block_dims_fixup, 0, stream>>>
...
void ds4n_mul_mat_q_switch_J(ggml_backend_cuda_context & ctx, const ds4n_mmq_args & args, cudaStream_t stream) {
    const int    id    = ggml_cuda_get_device();
    const int    cc    = ggml_cuda_info().devices[id].cc;
    ...
    for (int J = 8; J <= 128 && ntiles_J_best > 1; J += 8) {
        ...
        if (ds4n_mmq_get_nbytes_shared(config, cc) > smpbo) {
            continue;
        }
    ...
    switch (J_best) {
        case   8:
            ds4n_launch_mul_mat_q<type,   8, fallback>(ctx, args, stream);
            break;
        case  16:
            ds4n_launch_mul_mat_q<type,  16, fallback>(ctx, args, stream);
            break;
        case  24:
            ds4n_launch_mul_mat_q<type,  24, fallback>(ctx, args, stream);
            break;
        case  32:
            ds4n_launch_mul_mat_q<type,  32, fallback>(ctx, args, stream);
            break;
        case  40:
            ds4n_launch_mul_mat_q<type,  40, fallback>(ctx, args, stream);
            break;
        case  48:
            ds4n_launch_mul_mat_q<type,  48, fallback>(ctx, args, stream);
            break;
        case  56:
            ds4n_launch_mul_mat_q<type,  56, fallback>(ctx, args, stream);
            break;
        case  64:
            ds4n_launch_mul_mat_q<type,  64, fallback>(ctx, args, stream);
            break;
        case  72:
            ds4n_launch_mul_mat_q<type,  72, fallback>(ctx, args, stream);
            break;
        case  80:
            ds4n_launch_mul_mat_q<type,  80, fallback>(ctx, args, stream);
            break;
        case  88:
            ds4n_launch_mul_mat_q<type,  88, fallback>(ctx, args, stream);
            break;
        case  96:
            ds4n_launch_mul_mat_q<type,  96, fallback>(ctx, args, stream);
            break;
        case 104:
            ds4n_launch_mul_mat_q<type, 104, fallback>(ctx, args, stream);
            break;
        case 112:
            ds4n_launch_mul_mat_q<type, 112, fallback>(ctx, args, stream);
            break;
        case 120:
            ds4n_launch_mul_mat_q<type, 120, fallback>(ctx, args, stream);
            break;
        case 128:
            ds4n_launch_mul_mat_q<type, 128, fallback>(ctx, args, stream);
            break;
        default:
...
void ds4n_mul_mat_q_case(ggml_backend_cuda_context & ctx, const ds4n_mmq_args & args, cudaStream_t stream) {
    if (args.nrows_x % 128 == 0) {
        constexpr bool fallback = false;
        ds4n_mul_mat_q_switch_J<type, fallback>(ctx, args, stream);
    } else {
...
#define ds4n_DECL_MMQ_CASE(type)                                                        \
    template void ds4n_mul_mat_q_case<type>(ggml_backend_cuda_context & ctx, const ds4n_mmq_args & args, cudaStream_t stream) \

extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q1_0);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q2_0);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q4_0);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q4_1);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q5_0);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q5_1);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q8_0);
// -----------------------------------------
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q2_K);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q3_K);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q4_K);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q5_K);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_Q6_K);
// -----------------------------------------
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ1_S);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ2_XXS);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ2_XS);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ2_S);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ3_XXS);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ3_S);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ4_NL);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_IQ4_XS);
// -----------------------------------------
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_MXFP4);
extern ds4n_DECL_MMQ_CASE(GGML_TYPE_NVFP4);
...
bool ds4n_should_use_mmq(enum ggml_type type, int cc, int64_t ne11, int64_t n_experts);
